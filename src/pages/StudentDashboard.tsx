...
import { LogOut, Bell, TrendingUp } from "lucide-react";
...
const StudentDashboard = () => {
  ...
  const quickActions = [
    ...
    { 
      ...
      color: "bg-warning", 
      action: () => setIsMentorshipOpen(true) 
    },
  ];
      ...
      <main className="pt-24 pb-16">
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            {/* Welcome Header */}
            <div className="glass-card rounded-2xl p-4 md:p-8 mb-8 relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-6 mb-8">
              {quickActions.map((action, index) => (
                <div
                  key={index}
                  className="glass-card rounded-xl p-4 sm:p-6 hover-lift cursor-pointer group"
                  onClick={() => action.path ? navigate(action.path) : action.action?.()}
                >
                  <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg ${action.color} flex items-center justify-center mb-3 sm:mb-4 group-hover:shadow-glow transition-shadow`}>
                    {action.icon()}
                  </div>
                  <h3 className="font-semibold text-sm sm:text-lg">{action.label}</h3>
                </div>
              ))}
            </div>
            ...
            <div className="glass-card rounded-xl p-4 sm:p-6 mb-8">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-xl sm:text-2xl font-bold">This Week's Progress</h2>
              </div>
                ...
                {weeklyStats.map((stat, index) => (
                  <div key={index} className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs sm:text-sm text-muted-foreground">{stat.value}/{stat.max}</span>
                    </div>
